// src/core/agent-runtimes/gemini-headless-runtime.ts
...
/**
 * Gemini Headless Runtime Implementation
 *
 * Executes agents via the `gemini` CLI in non-interactive mode using `-p`.
 * Streams JSON output and extracts final text from stream events.
 */
export class GeminiHeadlessRuntime implements AgentRuntime {
  readonly type = 'gemini-headless';
  readonly name = 'Gemini Headless Mode';
  ...
  async validate(): Promise<ValidationResult> {
    try {
      await this.executeGeminiCLI(['--version'], { timeout: 5000 });
      return {
        valid: true,
        errors: [],
        warnings: []
      ...
      return {
        valid: false,
        errors: [`Gemini CLI not found or not working: ${error.message}`],
        warnings: [
  ...
  private buildCliArgs(request: AgentExecutionRequest, prompt: string): string[] {
    ...
    if (runtimeOpts.yolo === true) {
      args.push('--yolo');
    }
    if (runtimeOpts.approvalMode && typeof runtimeOpts.approvalMode === 'string') {
      args.push('--approval-mode', runtimeOpts.approvalMode);
    }
    if (runtimeOpts.sandbox && typeof runtimeOpts.sandbox === 'string') {
      args.push('--sandbox', runtimeOpts.sandbox);
    }
    if (runtimeOpts.debug === true) {
      args.push('--debug');
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        child.on('exit', (code) => {
          ...
          if (timedOut) {
            reject(new Error(`Gemini CLI execution timed out after ${timeout}ms. Process was terminated.`));
            return;
          }
          ...
          if (code === 0) {
            ...
            reject(
              new Error(
                `Gemini CLI exited with code ${code}. stderr: ${stderr || '(empty)'}${stdoutPreview}`
              )
            );
          }
        });
  ...
  private extractTextFromEvent(event: any): string | undefined {
    // Check for result-type events
    if (typeof event?.result === 'string') return event.result;
    if (typeof event?.response?.text === 'string') return event.response.text;
