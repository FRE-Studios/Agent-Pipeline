...
> Intelligent multi-runtime agent orchestration with DAG-planned parallelism, conditional logic, automated git hygiene, and multi-channel notifications. Supports Claude Code, Codex, Antigravity, and Pi Agent (15+ providers with built-in tools).
...
- **At least one agent runtime:**
  ...
  - **Antigravity** (`agy` CLI) – Google's Antigravity, serving Gemini, Claude, and GPT-OSS models
    - Authenticates via Google sign-in on first run; API keys are not supported
...
- **Runtime flexibility** – Pluggable agent runtimes (Claude Code Headless, Claude SDK, Codex Headless, Antigravity Headless, Pi Agent) registered via `AgentRuntimeRegistry`.
...
- `src/core/agent-runtime-registry.ts` – Registry for pluggable agent runtimes (Claude Code Headless, Claude SDK, Codex Headless, Antigravity Headless, Pi Agent).
